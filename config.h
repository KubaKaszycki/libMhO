...
/*
 * Define to 1 if fflush is known to work on stdin as per POSIX.1-2008, 0
 * if fflush is known to not work, -1 if unknown. 
 */
 ...
 * depending whether the gnulib module fflush shall be considered present. 
 */
#define GNULIB_FFLUSH 1

/*
 * Define to a C preprocessor expression that evaluates to 1 or 0,
 * depending whether the gnulib module fscanf shall be considered present. 
 */
#define GNULIB_FSCANF 1

/*
 * Define to a C preprocessor expression that evaluates to 1 or 0,
 * depending whether the gnulib module lock shall be considered present. 
 */
#define GNULIB_LOCK 1

/*
 * Define to a C preprocessor expression that evaluates to 1 or 0,
 * depending whether the gnulib module malloc-gnu shall be considered
 * present. 
 */
...
/*
 * Define to a C preprocessor expression that evaluates to 1 or 0,
 * depending whether the gnulib module scanf shall be considered present. 
 */
...
/*
 * Define to 1 if you have the declaration of `_snwprintf', and to 0 if
 * you don't. 
 */
 ...
 * automatically deduced at runtime. STACK_DIRECTION > 0 => grows toward
 * higher addresses STACK_DIRECTION < 0 => grows toward lower addresses
...
#endif
/*
 * Enable X/Open extensions if necessary.  HP-UX 11.11 defines mbstate_t
 * only if _XOPEN_SOURCE is defined to 500, regardless of whether
 * compiling with -Ae or -D_HPUX_SOURCE=1.  
 */
 ...
 * mishandles inline functions that call each other.  E.g., for 'inline
 * void f (void) { } inline void g (void) { f (); }', c99 incorrectly
 * complains 'reference to static identifier "f" in extern inline
 * function'. This bug was observed with Sun C 5.12 SunOS_i386 2011/11/16.
 * 
 * Suppress extern inline (with or without __attribute__
 * ((__gnu_inline__))) on configurations that mistakenly use 'static
 * inline' to implement functions or macros in standard C headers like
 * <ctype.h>.  For example, if isdigit is mistakenly implemented via a
 * static inline function, a program containing an extern inline function
 * that calls isdigit may not work since the C standard prohibits extern
 * inline functions from calling static functions.  This bug is known to
 * occur on:
 * 
 ...
 * and for clang but remains for g++; see
 * <http://trac.macports.org/ticket/41033>. Assume DragonFly and FreeBSD
 * will be similar.  
 */
...
/*
 * In GCC 4.6 (inclusive) to 5.1 (exclusive), suppress bogus "no previous
 * prototype for 'FOO'" and "no previous declaration for 'FOO'"
 * diagnostics, when FOO is an inline function in the header; see
 ...
 * be used.  This helps to reduce warnings, such as from GCC
 * -Wunused-parameter.  
 */
#ifndef _GL_UNUSED
 ...
 * supports the ISO C 99 semantics of 'extern inline' (unlike the GNU C
 * semantics of earlier versions), but does not display it by setting
 * __GNUC_STDC_INLINE__. __APPLE__ && __MACH__ test for Mac OS X.
...
/*
 * gcc supports the "unused" attribute on possibly unused labels, and g++
 * has since version 4.5.  Note to support C++ as well as C,
